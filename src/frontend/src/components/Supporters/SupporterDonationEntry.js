...
const SupporterDonationEntry = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        await Promise.all([fetchSupporters(), fetchDonationTypes()]);
      } catch (error) {
  ...
  const fetchSupporters = async () => {
    try {
      ...
      setSupporters(response.data.map(v => ({
        value: v.id,
        label: `${v.supporterNumber} - ${v.lastName}, ${v.firstName}`
      })));
    } catch (error) {
    ...
    try {
      ...
      const promises = donationRows.map(row =>
        axios.post(
          ...
          {
            supporter_id: selectedSupporter.value,
      ...
      alert(`Successfully added ${donationRows.length} donation(s) for ${selectedSupporter.label}`);
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-4xl">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Supporter Donation Entry</h1>
      ...
      {formLoading ? (
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
          ...
          {/* Supporter Name */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">Supporter Name</label>
            <Select
              options={supporters}
 ...
 export default SupporterDonationEntry;
