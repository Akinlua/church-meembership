...
const VisitorDonationEntry = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        await Promise.all([fetchVisitors(), fetchDonationTypes()]);
      } catch (error) {
    ...
    try {
      ...
      const promises = donationRows.map(row =>
        axios.post(
          ...
          {
            visitor_id: selectedVisitor.value,
      ...
      alert(`Successfully added ${donationRows.length} donation(s) for ${selectedVisitor.label}`);
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-4xl">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Visitor Donation Entry</h1>
...
export default VisitorDonationEntry;
